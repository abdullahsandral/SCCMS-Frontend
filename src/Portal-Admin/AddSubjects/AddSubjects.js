...
const Subjects = () => {
    ...
    const classChangeHandler = () => {
        dispatch(getOneClassSubjects(selectedClass?.id));
    }
    return <div className='background'>
        {(isEmpty(classes) && classes_loading) || ((isEmpty(class_subjects) && class_subjects_loading)) ?
            <LoadingSpinner name={classes_loading ? 'Classes Loading' : 'Subjects Loading'} /> :
        <div className='allForms'>
            <form>
                <h4 className='text-center gold'>Select a Class to Edit Subjects</h4>
                <div className="dropdown bg-white">
                    ...
                    <div className="dropdown-menu w-100 mt-0 p-0" aria-labelledby="dropdownMenuButton">
                        {classes?.map((cls, index) =>
                            <h6 key={index}
                                className={`m-0 p-2 px-4 item ${setSelectedClass?.id === cls?.id ? '.active_item' :''}`}
                    ...
                    <button disabled={!selectedClass} className='btn btn-success w-100 mt-3' onClick={classChangeHandler}>
                         {selectedClass?.name ? `View ${selectedClass?.name} Class Subjects` : 'Select a Class'}
                    </button>
            </form>
            {selectedClass && !isEmpty(class_subjects) && <div className='row m-3 allForms'>
                <h5 className='w-50 p-3 gold text-center custom-shadow '>Subject</h5>
                <h5 className='w-50 p-3 gold text-center custom-shadow '>Teacher</h5>
                {class_subjects?.map( subject => <>
                    <h6 className='w-50 p-3 gold text-center custom-shadow '>{subject?.name}</h6>
                    <h6 className='w-50 p-3 gold text-center custom-shadow '>{subject?.teacher?.first_name + ' ' + subject?.teacher?.last_name}</h6>
                </>)
